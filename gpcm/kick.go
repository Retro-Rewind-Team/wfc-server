...
func kickPlayer(profileID uint32, reason string, message WWFCErrorMessage) error {
	...
	if session, exists := sessions[profileID]; exists {
		...
		kickInfo = append(kickInfo, qr2.PIDIPPair{profileID, uint32(ip)})
		...
		for _, match := range findMatchingSessions(session) {
			...
			kickInfo = append(kickInfo, qr2.PIDIPPair{match.User.ProfileId, uint32(matchIP)})
		...
		kickInfo = append(kickInfo, qr2.PIDIPPair{user.ProfileId, uint32(ip)})
	}
